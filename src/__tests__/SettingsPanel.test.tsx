...
describe('SettingsPanel', () => {
  ...
  describe('sidebar position', () => {
    it('renders with left selected by default', () => {
      const {getByTestId} = render(<SettingsPanel onClose={noop} onAddDevice={noop} />);
  ...
  describe('paired devices', () => {
    ...
    it('removes device on Remove tap', () => {
      ...
      expect(useSettingsStore.getState().pairedDevices).toHaveLength(0);
    });

    it('calls onAddDevice when Add Device tapped', () => {
      const onAddDevice = jest.fn();
      const {getByTestId} = render(<SettingsPanel onClose={noop} onAddDevice={onAddDevice} />);
  ...
  describe('close', () => {
    it('calls onClose when ✕ tapped', () => {
      ...
      const {getByTestId} = render(<SettingsPanel onClose={onClose} onAddDevice={noop} />);
